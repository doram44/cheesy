...
Environment.ExecutorName = ExecutorName
Environment.IsAvailable = function(Name)
...
RunTest("writefile", function()
	Global.writefile("Cheesy_Test_File", "example")
	assert(Global.isfile("Cheesy_Test_File") == true, "Failed to create a file")
	assert(Global.readfile("Cheesy_Test_File") == "example", "File does not contain expected data")
end)

RunTest("isfile", function()
	assert(Global.isfile("Cheesy_Test_File") == true, "Did not return true for a valid file")
end)

RunTest("readfile", function()
	assert(Global.readfile("Cheesy_Test_File") == "example", "Did not return the expected data")
end)

RunTest("appendfile", function()
	Global.appendfile("Cheesy_Test_File", "_appended")
	assert(Global.readfile("Cheesy_Test_File") == "example_appended", "Failed to append content to a file")
end)

RunTest("loadfile", function()
	Global.writefile("Cheesy_Test_Load", [[return 25]])
	assert(Global.loadfile("Cheesy_Test_Load")() == 25, "Failed to load and execute a file")
end)

RunTest("delfile", function()
	Global.delfile("Cheesy_Test_File")
	Global.delfile("Cheesy_Test_Load")
	assert(Global.isfile("Cheesy_Test_File") == false, "Failed to delete a file")
end)

RunTest("makefolder", function()
	Global.makefolder("Cheesy_Test_Folder")
	assert(Global.isfolder("Cheesy_Test_Folder") == true, "Failed to create a folder")
end)

RunTest("delfolder", function()
	Global.delfolder("Cheesy_Test_Folder")
	assert(Global.isfolder("Cheesy_Test_Folder") == false, "Failed to delete a folder")
end)

RunTest("listfiles", function()
	Global.makefolder("Cheesy_ListFiles_Test")
	Global.writefile("Cheesy_ListFiles_Test/Test1", "test 1")
	Global.writefile("Cheesy_ListFiles_Test/Test2", "test 2")
	local FilesList = Global.listfiles("Cheesy_ListFiles_Test")
	local Found1 = false
	local Found2 = false
	...
	Global.delfolder("Cheesy_ListFiles_Test")
...
RunTest("getcustomasset", function()
	...
	local Content = game:HttpGet("https://raw.githubusercontent.com/deividcomsono/Obsidian/refs/heads/main/assets/Logo.png")
	Global.writefile("Cheesy_Test_Image", Content)
	local Asset = Global.getcustomasset("Cheesy_Test_Image")
	...
	TestImage:Destroy()
	Global.delfile("Cheesy_Test_Image")
